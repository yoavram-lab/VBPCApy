...
def trace_rms_within_iteration():
    ...
    # Replace individual phases to capture intermediate RMS
    orig_bias_center = pf._bias_and_center
    orig_score_rotate = pf._score_and_rotate
    orig_loadings = pf._update_loadings_phase
    orig_rms = pf._rms_phase
    orig_noise = pf._noise_phase

    step_traces = []

    def quick_rms(ctx):
        """RMS from current x_data, A, S."""
        x = ctx.centering_state.x_data
        a = ctx.training.model.a
        s = ctx.training.model.s
        mask = ctx.prepared.mask
        resid = np.asarray(x) - a @ s
        err = resid * np.asarray(mask, dtype=float)
    ...
    def substep_trace(ctx):
        ...
        # Snapshot A,S before scores
        a_pre_score = ctx.training.model.a.copy()
        s_pre_score = ctx.training.model.s.copy()
        ...
        # Run convergence
        convmsg, _ = pf._convergence_phase(ctx, x_data, rms_val, prms_val, s_xv)
